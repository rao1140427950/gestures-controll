...
# Training
t0=time.time()
xx=np.linspace(1,EPOCH,int(EPOCH/TEST_EPOCH))
yy=np.linspace(0,0,int(EPOCH/TEST_EPOCH))
yy2=np.linspace(0,0,int(EPOCH/TEST_EPOCH))
...
with tf.Session() as sess:
	...
	for i in range(1,EPOCH):
		...
		if i%TEST_EPOCH == 0:
			...
			yy[int(i/TEST_EPOCH)]=train_accuracy1
			yy2[int(i/TEST_EPOCH)]=train_accuracy2

	print('Training Finished!')
	print('Average accuracy:',np.average(yy[-20:-1]),np.average(yy2[-20:-1]))
